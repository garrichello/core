...
class DataAccess():
    ...
    def _get_metadata(self, metadb_info, argument):
        ...
        qry = qry.join(levels_variable_tbl)
        ...
        qry = qry.filter(collection_i18n_tbl.columns['lang_code'] == ENGLISH_LANG_CODE)
        qry = qry.filter(parameter_i18n_tbl.columns['lang_code'] == ENGLISH_LANG_CODE)
        qry = qry.filter(units_i18n_tbl.columns['lang_code'] == ENGLISH_LANG_CODE)
