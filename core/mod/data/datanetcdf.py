...
class DataNetcdf(Data):
    ...
    def write_array(self, all_values, all_options_dict):
        """Writes data array into a netCDF file.

        Arguments:
            all_values -- processing result's values as a list of masked array/array/list.
            all_options_dict -- dictionary of lists of write options:
                ['level'] -- vertical level name
                ['segment'] -- time segment description (as in input time segments taken from a task file)
                ['times'] -- time grid as a list of datatime values
                ['longitudes'] -- longitude grid (1-D or 2-D) as an array/list
        ...
        root.Title = all_options_dict['description']['@title']
        root.Conventions = 'CF'
        root.Source = 'Generated by CLIMATE Web-GIS'

        # Define geographic variables.
        lon = root.createDimension('lon', all_options_dict['longitudes'].size)  # pylint: disable=W0612
        longitudes = root.createVariable('lon', 'f4', ('lon'))
        lat = root.createDimension('lat', all_options_dict['latitudes'].size)  # pylint: disable=W0612
        ...
        longitudes[:] = all_options_dict['longitudes']
        latitudes[:] = all_options_dict['latitudes']


        # In multivalues case all_options_dict is a dictionary of lists. Convert it to a list of dictionaries.
        all_options = self._transpose_dict(all_options_dict)

        # All values MUST have the same variable name in options!
        common_meta = all_options[0].get('meta')  # Take the first options to get the common meta.
        varname = None if common_meta is None else common_meta.get('varname')
        varname = 'data' if varname is None else varname

        # Prepare values and grids for writing.
...
#            if new_file:
            ...
            if len(options['times']) > 0:  # If time grid is present.
            ...
            if level_value:
                if 'level' not in root.variables:  # If file is new or does not contain level variable.
