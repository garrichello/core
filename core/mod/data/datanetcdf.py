...
class DataNetcdf(Data):
    ...
    def write_array(self, all_values, all_options):
        ...
        for level in all_options['level']:
            ...
            if level_value:
                levels['value'].append(int(level_value[0]))
            level_units = re.findall(r'[a-zA-Z]+', level)  # Take an alpha part.
            if level_units:
                levels['units'].append(level_units[0])
        ...
        if len(all_options['times']) > 0:  # If time grid is present.
            # Define time variable.
            dim_list.insert(0, 'time')  # Add dimension time.
            time = root.createDimension('time', len(all_options['times']))  # pylint: disable=W0612
            times = root.createVariable('time', 'f8', ('time'))
            # Set time attributes.
            times.units = 'days since {}-1-1 00:00:0.0'.format(all_options['times'][0].year)
            ...
            # Write time variable.
            start_date = datetime(all_options['times'][0].year, 1, 1)
            times[:] = [(cur_date - start_date).days for cur_date in all_options['times']]
